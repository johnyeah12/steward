...
function renderUptown() {
  ...
  renderUptownSummary(all, mine);
  renderUptownSetup(all);
 ...
 */
/**
 * The flat as an investment, kept apart from any single month: what went in to
 * get it ready, and what it gives back.
 *
 * A mortgage payment is financing, not a cost of letting — it buys equity and
 * falls due whether or not there is a guest. It is left out of every figure
 * here; counting it would pin payback at zero for the life of the loan.
 */
const isFinancing = t => /home\s*loan|mortgage/i.test(t.note || '');

function renderUptownSummary(all, running) {
  const box = $('#upSummary');
  ...
  if (!spent) {
    box.innerHTML = `<p class="card-note" style="margin:0">
      Once you've recorded what it cost to set the flat up, this shows how fast
      it is earning that back.</p>`;
    return;
  }
  ...
  /* Projected on gross — what guests paid, before running costs. A blunter
     measure than net, but a thin month of bills can't swing it. Only complete
     months count: a half-finished month always reads as a bad one. */
  const thisMonth = monthKey(todayISO());
  const gross = new Map();
  ...
  const perMonth = live.length
    ? [...gross.values()].reduce((s, v) => s + v, 0) / live.length : 0;
  const months = perMonth > 0 ? spent / perMonth : Infinity;
  const yearly = perMonth > 0 ? (perMonth * 12) / spent * 100 : 0;

  box.innerHTML = `
    <div class="ytd-row"><span class="ytd-month">Put in to set it up</span>
      <span class="ytd-net">${moneyProp(spent)}</span></div>
    ${perMonth > 0 ? `
    <div class="ytd-row"><span class="ytd-month">Gross earnings a month</span>
      <span class="ytd-net">${moneyProp(Math.round(perMonth))}</span></div>
    <div class="ytd-row"><span class="ytd-month">Fit-out back in</span>
      <span class="ytd-net">${months < 120 ? `${Math.ceil(months)} months` : 'not at this rate'}</span></div>
    <div class="ytd-row ytd-total"><span>Return a year, on gross</span>
      <span class="ytd-net">${yearly.toFixed(0)}%</span></div>` : ''}
    ...
    <p class="card-note" style="margin:10px 0 0">
      ...
      ${perMonth > 0 ? `Projected from ${live.length} complete month${live.length === 1 ? '' : 's'}
      of letting${live.length < 3 ? ' — too few to lean on yet' : ''}.` : ''}
      The bar counts what the flat actually nets after running costs, so it moves slower
      than the gross figure. The home loan is left out of both — it buys equity rather
      than being a cost of letting.
    </p>`;
}

function renderUptownSetup(all) {
  const box = $('#upSetup');
  const setup = all.filter(isSetup);
  ...
  box.innerHTML = `
    ...
    <p class="card-note" style="margin:10px 0 0">
      ${setup.length} one-off cost${setup.length === 1 ? '' : 's'}, kept out of the monthly
      figures so a single purchase doesn't distort a month. How fast this is coming back
      is in the summary above.
    </p>`;
}
